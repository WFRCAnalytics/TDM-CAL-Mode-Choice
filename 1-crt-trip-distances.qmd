...
```{python}
# What columns have non-null values for this trip?
df_plot[df_plot.filter(regex='(PREV|NEXT|STOP).*(_LAT|_ON_BUS|_OFF_BUS)').columns].notna()
```
...
MODE_STYLES = {
    ...
    7: {'color': '#ce132d', 'weight': 6},   # LRT (Light Rail) - UTA Red
    8: {'color': '#9c4392', 'weight': 8},   # CRT (Commuter Rail) - UTA Green
    9: {'color': '#007dbb', 'weight': 5}    # BRT (Bus Rapid Transit)
}

ACCESS_EGRESS_STYLES = {
    'Walk': {'color': '#898a8d', 'weight': 1.5},
    'KNR': {'color': '#FFA500', 'weight': 2},
    'PNR': {'color': '#FF6B35', 'weight': 2},
    'Drive': {'color': '#FF6B35', 'weight': 2}
}

TRANSFER_WALK_STYLE = {'color': '#898a8d', 'weight': 1.5}
