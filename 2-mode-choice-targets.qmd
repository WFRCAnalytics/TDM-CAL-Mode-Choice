...
```{python}
USE_CRT_CLASSES = True  # Set to False to use single CRT category

# Determine which column to use for mode analysis
MODE_COLUMN = 'Linked_Mode_txt_crt' if USE_CRT_CLASSES else 'Linked_Mode_txt'
```
...
```{python}
# Auto-generate CRT mappings based on available data
unique_modes = df_obs_linked[MODE_COLUMN].dropna().unique()
crt_categories = sorted([mode for mode in unique_modes if mode.startswith('CRT')])
```
...
for crt_cat in crt_categories:
    ...
    service_mapping[crt_cat] = f'crt{suffix}'

# 6. Transit by Service Type
df_transit_by_service = calculate_categorical_shares(
    df_obs_linked,
    category_col=MODE_COLUMN,
    weight_col='trip_weight',
    group_cols=['Veh_Cat3p', 'Purp5_text', 'PK_OK'],
    filter_condition=f'{MODE_COLUMN} != "MT"',
...
df_walk_to_transit = calculate_categorical_shares(
    ...
    weight_col='trip_weight',
    group_cols=['Veh_Cat3p', 'Purp5_text', 'PK_OK'],
    filter_condition=f'Ac_Mode2_Model == "Walk" & {MODE_COLUMN} != "MT"',
...
for crt_cat in crt_categories:
    suffix = '' if crt_cat == 'CRT' else crt_cat.split('_')[1].lower()
    drive_service_mapping[crt_cat] = f'dcrt{suffix}'

# 9. Drive-to-Transit by Service Type
df_drive_to_transit = calculate_categorical_shares(
    df_obs_linked,
    category_col=MODE_COLUMN,  # Changed
    weight_col='trip_weight',
    group_cols=['Veh_Cat3p', 'Purp5_text', 'PK_OK'],
    filter_condition=f'Ac_Mode2_Model == "Drive" & {MODE_COLUMN} != "MT"',  # Updated
...
def create_tree_dataframe(
    ...
    # Dynamically add all walk-to-transit service types
    for service_code, service_var in walk_service_mapping.items():
        service_share = get_value(df_walk_to_transit, service_var, purpose, veh_cat, period)

        # Determine display name
        if service_code == 'CRT':
            display_name = 'CRT'
        elif service_code.startswith('CRT_'):
            display_name = service_code  # e.g., 'CRT_00', 'CRT_01'
        else:
            display_name = service_code  # e.g., 'Local Bus', 'LRT'

        rows.append({
            'purpose': purpose, 'period': display_period, 'veh_cat': veh_cat,
            'level1': 'Motorized', 'level2': 'Transit', 'level3': 'Walk-to-Transit', 'level4': display_name,
    ...
    # Dynamically add all drive-to-transit service types
    for service_code, service_var in drive_service_mapping.items():
        service_share = get_value(df_drive_to_transit, service_var, purpose, veh_cat, period)
        ...
        rows.append({
            'purpose': purpose, 'period': display_period, 'veh_cat': veh_cat,
            'level1': 'Motorized', 'level2': 'Transit', 'level3': 'Drive-to-Transit', 'level4': display_name,
